const PREVIEW_ROOT=new URL('./',import.meta.url);
const SITE_ROOT=new URL('../',PREVIEW_ROOT);
...
<footer><span>${esc(site.copyright)}</span></footer>`;
  setupTheme();
  setupQr(new URL('i18n-preview/'+activeLocale+'/',site.url).href);
  setupContact();
  setupAlpacaReveal();
...
function setupLanguageSwitcher(locale,copy){
  ...
  menu.querySelectorAll('[data-locale]').forEach(button=>button.onclick=()=>setLocale(button.dataset.locale,{persist:true,history:'push'}));
