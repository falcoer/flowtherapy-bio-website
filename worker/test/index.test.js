...
const worker=createWorker(FakeEmailMessage);

const valid=(overrides={})=>({subject:'booking',email:'artist@example.com',phone:'+33 6 00 00 00 00',message:'Bonjour, nous souhaitons programmer Flow Therapy.',company:'',startedAt:String(Date.now()-5000),...overrides});
...
test('sends a valid message to the contact alias',async()=>{
  let sent;
  const env={CONTACT_RATE_LIMIT:{limit:async()=>({success:true})},EMAIL:{send:async message=>{sent=message;return {messageId:'test'}}}};
...
test('rate limits before sending',async()=>{
  const env={CONTACT_RATE_LIMIT:{limit:async()=>({success:false})},EMAIL:{send:async()=>assert.fail('must not send')}};
